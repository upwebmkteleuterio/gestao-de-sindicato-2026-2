...
const JuridicoControls = () => {
  const currentISO = useAppStore((state) => state.ui.currentJuridicoDate);
  const setJuridicoDate = useAppStore((state) => state.setJuridicoDate);
  ...
  const handlePrevWeek = () => setJuridicoDate(subDays(date, 7));
  const handleNextWeek = () => setJuridicoDate(addDays(date, 7));
        ...
        <Popover>
          ...
          <PopoverContent className="w-auto p-0" align="center" sideOffset={8}>
            <Calendar
              mode="single"
              selected={date}
              onSelect={(d) => d && setJuridicoDate(d)}
              locale={ptBR}
              initialFocus
      ...
      <div className="flex gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 no-scrollbar">
        <div className="flex items-center gap-4 bg-white border border-slate-200 px-4 py-1.5 rounded-lg shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase">Agendado</span>
          </div>
          <div className="flex items-center gap-2">
...
export default JuridicoControls;
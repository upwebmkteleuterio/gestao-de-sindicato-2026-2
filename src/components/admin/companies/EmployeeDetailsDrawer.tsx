...
const EmployeeDetailsDrawer: React.FC<EmployeeDetailsDrawerProps> = ({ employee, onClose }) => {
  ...
  const { data: dependents = [], isLoading: isLoadingDependents } = useQuery({
    queryKey: ["admin-employee-dependents", employee?.id],
    queryFn: async () => {
      if (!employee?.id) return [];
      const { data, error } = await supabase
  ...
  return (
    <>
      ...
      <div className={cn(
        "fixed top-0 right-0 h-full w-full max-w-xl bg-white shadow-2xl z-[70] border-l border-slate-200 flex flex-col transition-transform duration-500 ease-in-out",
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 no-scrollbar">
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-4">
              ...
              <DetailItem label="E-mail de Contato" value={displayEmployee.email} />
            </div>
          </section>
          ...
          <section className="space-y-6">
            <div className="flex items-center gap-2 text-slate-900 border-b border-slate-100 pb-2">
              ...
              <h4 className="text-xs font-black uppercase tracking-widest">Dados Profissionais & Sindicato</h4>
            </div>
            <div className="grid grid-cols-2 gap-y-6 gap-x-4">
              <DetailItem label="Cargo Atual" value={displayEmployee.role} />
              <DetailItem label="CTPS / Série" value={displayEmployee.ctps} />
              <DetailItem label="Salário Base" value={displayEmployee.salary ? `R$ ${displayEmployee.salary}` : "---"} />
              <DetailItem label="Data de Admissão" value={displayEmployee.admission_date ? new Date(displayEmployee.admission_date).toLocaleDateString('pt-BR') : "---"} />
                ...
                <div className={cn(
                  ...
                  <div>
                    ...
                    <p className={cn("text-[11px] font-medium", displayEmployee.status === "Associado" ? "text-emerald-700" : "text-slate-500")}>
                      {displayEmployee.status === "Associado" ? "Filiado e contribuindo regularmente." : "Funcionário não associado."}
                    </p>
                  </div>
                </div>
              </div>
          ...
          {/* SEÇÃO 4: HISTÓRICO DE AGENDAMENTOS */}
            ...
            <div className="space-y-3">
              {isLoadingHistory ? (
                ...
                history.map((item: any) => (
                  <div key={item.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 flex items-center justify-between group hover:border-blue-200 transition-colors">
                    ...
                    <div className="text-right">
                      <p className="text-xs font-bold text-slate-700">{new Date(item.scheduled_date).toLocaleDateString('pt-BR')}</p>
                ...
                <div className="p-10 border-2 border-dashed border-slate-100 rounded-2xl text-center">
                  ...
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Sem registros de homologação ou jurídico</p>
                </div>
              )}
            </div>
          </section>
        ...
        <div className="p-6 border-t border-slate-200 bg-slate-50 flex items-center justify-center">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ficha de Auditoria Interna • Gestão Sindical</p>
        </div>
      </div>
    </>
  );
